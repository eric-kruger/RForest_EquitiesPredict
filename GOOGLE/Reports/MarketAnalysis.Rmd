...
# Load Trained Model
  load("~/R/RForest_EquitiesPredict/GOOGLE/GOOGL_10.20.17.Rdata")
...
# Create Prediction Data Frame (Table)
  prediction <- cbind.data.frame(Date=perf$predict.data$Date,
                                 Close=perf$predict.data$GOOGL.Close.raw,   # Need to build a function for this line
                                 perf$probs,
                                 perf$S.ST.B[,-4],
                                 perf$Performance)
...
write.csv(pred$predict.data,"~/R/RForest_EquitiesPredict/GOOGLE/HistoricalData/GOOGLE_pred_data.csv")
```
...
```{r}
GOOGL.ID <- Intraday("https://finance.google.com/finance/getprices?q=GOOGL&x=NASDAQ&i=60&p=1d&f=d,c,h,l,o,v")
plot(GOOGL.ID$CLOSE)
barChart(GOOGL.ID,theme='white.mono',bar.type='hlc')
```
