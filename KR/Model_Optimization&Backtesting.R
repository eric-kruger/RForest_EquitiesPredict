source("~/R/RForest_EquitiesPredict/KR/SETUP.R")
source(paths$path.lib)

# Load Data
quantmod::getSymbols(c("KR"),src="google",from ="2004-12-15",to = Sys.Date()) 
stock.data <- list(KR=KR)

# Model
mod <- stock.RF.mod(stock2model = "KR",
                    symbols=stock.data,
                    forecast.days = 20,
                    FROM = "2004-12-15",
                    TO = "2017-08-29",
...
for (i in 1:length(sim.s)) {
  mod <-   mod <- stock.RF.mod(stock2model = "KR",                     # Need to fix this model 
                               symbols=stock.data,
                               forecast.days = 20,
                               FROM = "2001-12-15",
                               TO = "2017-08-29",
